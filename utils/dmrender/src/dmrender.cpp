...
void writeWave(std::vector<std::int16_t>& wavedata, std::ostream& stream) {
    ...
    littleEndianWrite<std::uint32_t>(sizeof(WaveFormatEx), data); data += 4;
    littleEndianWrite<std::uint16_t>((std::uint16_t)(WaveFormatTag::PCM), data); data += 2;
    littleEndianWrite<std::uint16_t>(1, data); data += 2;
    littleEndianWrite<std::uint32_t>(44100, data); data += 4;
    littleEndianWrite<std::uint32_t>(88200, data); data += 4;
    littleEndianWrite<std::uint16_t>(2, data); data += 2;
    littleEndianWrite<std::uint16_t>(16, data); data += 2;
    littleEndianWrite<std::uint16_t>(0, data); data += 2;

    memcpy(data, "data", 4); data += 4;
    littleEndianWrite<std::uint32_t>(wavedata.size(), data); data += 4; // Padding byte

    memcpy(data, (const char*)(wavedata.data()), wavedata.size());
    data += wavedata.size();
    data += wavedata.size() % 2; // Padding byte
    ...
    stream.write((const char*)output.data(), output.size());
}
